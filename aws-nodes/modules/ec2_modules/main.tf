...
resource "aws_instance" "ise_node" {
  for_each      = local.ise_instance_config
  subnet_id     = var.subnet_id_list[index(keys(local.ise_instance_config), each.key) % length(var.subnet_id_list)]
