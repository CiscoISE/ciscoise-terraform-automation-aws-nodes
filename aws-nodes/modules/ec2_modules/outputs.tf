...
output "primary_instance_id" {
  ...
  value       = join("", [for key in keys(var.primary_instance_config) : aws_instance.ise_node[key].id])
}

output "secondary_instance_id" {
  description = "Instance id of the secondary ISE node"
  value       = join("", [for key in keys(var.secondary_instance_config) : aws_instance.ise_node[key].id])
}

output "psn_instance_id" {
  description = "Instance id of the PSN ISE nodes"
  value       = [for key in keys(var.psn_instance_config) : aws_instance.ise_node[key].id]
}

output "primary_private_ip" {
  description = "Private IP address of primary ISE node"
  value       = join("", [for key in keys(var.primary_instance_config) : aws_instance.ise_node[key].private_ip])
}

output "secondary_private_ip" {
  description = "Private IP address of Secondary ISE node"
  value       = join("", [for key in keys(var.secondary_instance_config) : aws_instance.ise_node[key].private_ip])
}

output "psn_private_ip" {
  description = "Private IP address of PSN ISE nodes"
  value       = [for key in keys(var.psn_instance_config) : aws_instance.ise_node[key].private_ip]
}
