...
resource "aws_launch_template" "ise_launch_template" {
  ...
  user_data              = templatefile("userdata.tftpl", { dns_domain = var.dns_domain, password = var.password, time_zone = var.time_zone, ers_api = var.ers_api, open_api = var.open_api, px_grid = var.px_grid, px_grid_cloud = var.px_grid_cloud })
